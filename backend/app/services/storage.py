"""Фасад хранилища артефактов: отчёты, CSV-датасеты, черновики скиллов.

Сейчас артефакты лежат на диске (режим local, каталог из env ARTIFACTS_DIR,
по умолчанию backend/artifacts — текущий лэйаут сохранён 1:1, поэтому
существующие отчёты и CSV работают без миграции). При переносе на S3
(ARTIFACTS_STORAGE=s3) достаточно добавить реализацию с тем же интерфейсом:
path/materialize будут выгружать файлы во временный каталог для
subprocess'ов, а каноничным хранилищем станет бакет.
"""
...
_SUBDIRS = {
    'report': '',           # <base>/<report_id>/report.py
    'csv': 'datasets',      # <base>/datasets/<slug>/data.csv
    'draft': 'skill_drafts',  # <base>/skill_drafts/<draft_id>/skill.md
}
