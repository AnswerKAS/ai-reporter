...
def resolve_dataset_dsn(dataset: dict) -> str:
    ...
    Поддержка:
    - литеральный DSN — единственный формат, который заводится сегодня;
    ...
    Два последних формата остаются **только ради записей, заведённых раньше**
    (включая дефолтные датасеты витрины со ссылкой `env:DATABASE_URL`): новых
    таких датасетов `api/datasets.py:_validate_dsn` больше не создаёт.
